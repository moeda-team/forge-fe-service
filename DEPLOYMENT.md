...
- `GET https://<forge-be>.vercel.app/health` menghasilkan status 200.
...
Variable `NEXT_PUBLIC_*` dimasukkan ke bundle browser ketika build. Jangan pernah menyimpan `GEMINI_API_KEY`, database URL, `JWT_SECRET`, atau secret lain di frontend.
...
2. Pastikan halaman Login/Register muncul. Jika langsung muncul data mock, `NEXT_PUBLIC_FORGE_API_URL` belum terbaca saat build.
...
- Isi `NEXT_PUBLIC_FORGE_API_URL` untuk environment Preview.
- Pastikan URL Preview frontend diizinkan oleh `FRONTEND_URL` backend Preview.
...
- Pastikan `NEXT_PUBLIC_FORGE_API_URL` tersedia pada environment deployment yang benar.
- Pastikan nilainya tidak berakhiran `/api`.
- Redeploy frontend setelah mengubah variable.

### `Failed to fetch`

- Buka backend `/health` secara langsung.
- Periksa browser Network dan Vercel Function logs.
- Periksa `FRONTEND_URL` pada backend.
...
- Periksa respons `/api/auth/login` pada browser Network.
...
- Periksa respons `/api/ai/chat` dan Vercel logs backend.
...
- Pastikan screen berhasil dibuat dan request `PUT /api/screens/:id/nodes` tidak error.
...
- [ ] `NEXT_PUBLIC_FORGE_API_URL` menunjuk backend Production.
